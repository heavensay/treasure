...
public class MyWebSocketInterceptor extends HttpSessionHandshakeInterceptor {
    ...
    public boolean beforeHandshake(ServerHttpRequest request, ServerHttpResponse response, WebSocketHandler wsHandler, Map<String, Object> attributes) throws Exception {
        logger.info("beforeHandshake " + request.getRemoteAddress().getHostString());
        if (request instanceof ServletServerHttpRequest){
            ...
            System.out.println("getQueryString:"+ JSON.toJSON(serverHttpRequest.getServletRequest().getQueryString()));
            System.out.println("getHeaderNames："+JSON.toJSON(serverHttpRequest.getServletRequest().getHeaderNames()));
            response.setStatusCode(HttpStatus.ACCEPTED);
            response.setStatusCode(HttpStatus.INSUFFICIENT_STORAGE);
            serverHttpResponse.getServletResponse().getWriter().write("dddd");
            serverHttpResponse.getServletResponse().addHeader("X-JJJJ","kkkk");
